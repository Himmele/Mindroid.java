 ...
 * work properly:</p>
 * <ul>
 *     <li>The task instance must be created on a looper thread.</li>
 *     <li>{@link #execute} must be invoked on a looper thread.</li>
...
public abstract class AsyncTask<Params, Progress, Result> {
    ...
    private static class SerialExecutor implements Executor {
        ...
        protected synchronized void scheduleNextRunnable() {
            if ((mRunnable = mTasks.poll()) != null) {
    ...
    public AsyncTask() {
        ...
        mFuture = new FutureTask<Result>(mWorker) {
            @Override
            protected void done() {
                final boolean wasInvoked = mInvoked.get();
                try {
                    Result result = get();
                    if (!wasInvoked) {
                        postResult(result);
                    ...
                    throw new RuntimeException("Failed to execute AsyncTask.doInBackground", e.getCause());
     ...
     * on its use.
     *
     * <p>This method must be invoked on the application main thread.
     *
     * @param params The parameters of the task.
     *
     ...
     * <p>This method must be invoked on the application main thread.
     *
     * @param executor The executor to use.  {@link #THREAD_POOL_EXECUTOR} is available as a
